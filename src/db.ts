...
export class GatewayDB {
  ...
  getInvoiceByMemo(memo: string): Invoice | undefined {
    return this.db.prepare("SELECT * FROM invoices WHERE memo = ?").get(memo) as Invoice | undefined;
  }
  ...
  addBalance(clientId: string, amount: string): string {
    ...
    `).run(clientId, newBalance, new Date().toISOString(), newBalance, new Date().toISOString());
    return newBalance;
  }
  ...
  logPayment(invoiceId: string | null, clientId: string | null, amount: string, direction: "in" | "out", route: string, txHash?: string): void {
    this.db.prepare(`
